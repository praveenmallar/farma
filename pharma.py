...
class Pharma(Tk):
    ...
    def addshortcuts(self):
        ...
        b = Button(f, image=photo, text="purchase", compound=BOTTOM, width=100, height=100,
                   command=lambda x=self: purchase.addStock(x))
        b.pack(side=LEFT)
        b.image = photo
        photo = PhotoImage(file="./images/return.png")
        b = Button(f, image=photo, text="cancel", compound=BOTTOM, width=100, height=100,
        ...
        b = Button(f, image=photo, text="patient", compound=BOTTOM, width=100, height=100,
    ...
    def dayreport(self):
        ...
        try:
            ...
            lastprint = 0
        shellsale = sh['bills']['sale']
        sheltotal = 0
        for sl in shellsale:
            sheltotal += Decimal(sl[1])
        print(sheltotal)
        lines.append("report from bill#" + str(lastprint + 1) + " to #" + str(lastbill))
        lines.append("")
        lines.append('sale     :' + "{0:.2f}".format(sh['sale']))
        if int(sh['sale']-sh['return']) != int(sheltotal):
            lines.append('sale [1] :' + "{0:.2f}".format(sheltotal))
        ...
        lines.append("")
        csvstring = "{},{},{},{},{},{},{},{},{}\n".format(str(dt.date.today()), lastprint + 1, lastbill, sh['sale'],
    ...
    def purchasepay(self):
        ...
        for stockist in stockists:
            sql = "select count(amount), sum(amount) ,group_concat(round(amount)) from purchase where date < date_format(now(),'%Y-%m-01') and paid!=1 and stockist=" + str(
                stockist[0]) + ";"
            cur.execute(sql)
            if cur.rowcount > 0:
                res = cur.fetchone()
                ...
                if count > 0:
                    ...
                    items.append(item)
                    sql = "update purchase set paid=1 where stockist=" + str(
                        stockist[0]) + " and date<date_format(now(),'%Y-%m-01');"
                    cur.execute(sql)
        con.commit()
