...
class Review(Frame):
    ...
    def showOptions(self, selection):
        ...
        if selection == "sale":
            ...
            Button(fr, text="Load", command=lambda g=fr.g, d=fr.d, date=date, doc=doc, dr=dr, d1=cb1, d2=cb2,
                                                   cnt=countoramount: self.showsale(g, d, date, doc, dr, d1, d2,
                                                                                    cnt)).pack(padx=20)
            ...
            Button(fr, text="Load", command=lambda g=fr.g, d=fr.d, d1=cb1, d2=cb2: self.showsale2(g, d, d1, d2)).pack(
                padx=20)
            ...
            opt.set(3)
            Button(fr, text="Load", command=lambda g=fr.g, d=fr.d, opt=fr.opt: self.showstock(g, d, opt)).pack(padx=20,
                                                                                                               pady=5)
            ...
            ft.timeperiod.set(3)
            Button(fr, text="Load", command=lambda g=fr.g, d=fr.d, st=st, t=ft: self.showpurchase(g, d, st, t)).pack(
                pady=5, padx=20)
    ...
    def showbills(self, d1, d2, op1):
        ...
        if not optdate:
            sql = "select bill.id, bill.name, bill.net, bill.date, doc.name from bill join doc on bill.doc=doc.id where bill.date >= str_to_date(\"{}\",\"{}\") and bill.date <= str_to_date(\"{}\",\"{}\") order by bill.id;".format(
                d1, "%d-%b-%y", d2, "%d-%b-%y")
            ...
            sql = "select bill.date, min(bill.id),max(bill.id),sum(bill.net) from bill where bill.date >= str_to_date(\"{}\",\"{}\") and bill.date <= str_to_date(\"{}\",\"{}\") group by bill.date order by bill.date;".format(
                d1, "%d-%b-%y", d2, "%d-%b-%y")
    ...
    def showpurchase(self, g, d, s, t):
        ...
        if dr > -1:
            sql = (
                "select stockist.name,purchase.date,stock.start_count,stock.buy_price,stock.price * (1-(stock.discount/100)+(stock.tax/100)) ")
            format = " {:15.15s} {:%b,%y} {:5.0f} {:7.2f} {:7.2f}"
            tf = ("stockist", "date", "count", "cost", "price")
            tl = " {:15.15s} {:6.6s} {:5.5s} {:7.7s} {:7.7s}".format(*tf)
        else:
            sql = (
                "select drug.name, sum(stock.start_count), avg(stock.buy_price),avg(stock.price * (1-(stock.discount/100)+(stock.tax/100)) )")
    ...
    def showstock(self, g, d, op):
        ...
        if checklow:
            ...
            sql = ("select drug.name,stock.cur_count,stock.expiry from drug join stock on"
				   ...
				   " and expiry > curdate() order by drug.name; ")
    ...
    def showsale(self, g, d, date, doc, dr, d1, d2, cnt):
        ...
        sql = "select {}, {} from drug join stock on drug.id=stock.drug_id join sale on sale.stock=stock.id join bill on sale.bill = bill.id join doc on doc.id=bill.doc where bill.date> str_to_date(\"{}\",\"{}\") and bill.date< str_to_date(\"{}\",\"{}\") {} {}  group by {} order by {} ;".format(
            selectstring, countstring, date1, '%d-%b-%y', date2, '%d-%b-%y', wheredrug, wheredoc, groupstring,
            dateorder)
    ...
    def showsale2(self, g, d, d1, d2):
        ...
        tl = "{:12.12s}-{:10.10s}-{:6.6s} ({:5.5s}) {:18.18s}-{:5.5s}".format("drug", "batch", "bill", "count",
                                                                              "patient", "date")
        if drug > -1:
            wheredrug = " and drug.id={} ".format(drug)
        elif group > -1:
        ...
        sql = "select drug.name,stock.batch,bill.id,sale.count,bill.name,bill.date from drug join stock on drug.id=stock.drug_id join sale on stock.id=sale.stock join bill on bill.id=sale.bill where bill.date> str_to_date(\"{}\",\"{}\") and bill.date< str_to_date(\"{}\",\"{}\") {} order by drug.name,sale.id".format(
            date1, '%d-%b-%y', date2, '%d-%b-%y', wheredrug)
