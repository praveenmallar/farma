...
class addNew(tk.Frame):
    ...
    def edit(self):
        ...
        if not askokcancel("Confirm", "Change " + oldtx + " to " + newtx + "?", parent=self.master):
            return
        cursor = self.db.cursor()
        sql = "update {3} set {2} =\"{0}\" where {2} = \"{1}\";".format(newtx, oldtx, self.field, self.table)
        print(sql)
...
class addDrug(addNew):
    ...
    def listboxchanged(self, e):
        ...
        cur.execute("select name from manufacture where id in (select manufacture from drug where name=\"{}\");".format(dr))
        if cur.rowcount == 1:
            r = cur.fetchone()
            r = r[0]
        else:
    ...
    def edit(self):
        ...
        if mfcr:
            ...
            mfcr = 0
        cur = self.db.cursor()
        sql="update drug set manufacture={} where name=\"{}\"".format(mfcr, drg)
        print(sql)
        cur.execute(sql)
        self.db.commit()
